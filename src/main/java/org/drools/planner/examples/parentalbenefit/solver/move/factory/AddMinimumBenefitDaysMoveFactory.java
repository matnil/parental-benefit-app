...
import org.drools.planner.examples.parentalbenefit.solver.move.UpdateDaysOnMinimumBenefitMove;

public class AddMinimumBenefitDaysMoveFactory extends AbstractMoveFactory {
    ...
    public List<Move> createMoveList(@SuppressWarnings("rawtypes") Solution solution) {
        ...
        for (ListIterator<MonthPlan> iter = monthPlanList.listIterator(); iter.hasNext();) {
            ...
            if (monthPlan.getDaysAtWork() > 0 &&
            	...
            	// replace one day at work with one day on minimum benefit
                moveList.add(new UpdateDaysOnMinimumBenefitMove(parentalBenefitPlan, monthPlan, 1));
            }
        }
        return moveList;
    }
