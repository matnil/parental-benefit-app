...
import org.drools.planner.examples.parentalbenefit.solver.move.UpdateDaysOnSicknessBenefitMove;

public class AddSicknessBenefitDaysMoveFactory extends AbstractMoveFactory {
    ...
    public List<Move> createMoveList(@SuppressWarnings("rawtypes") Solution solution) {
        ...
        for (ListIterator<MonthPlan> iter = monthPlanList.listIterator(); iter.hasNext();) {
            ...
            if (!parentalBenefitPlan.getPlanForOtherParent(monthPlan).isAtHomeDuringMonth() && monthPlan.getDaysAtWork() > 0) {
            	moveList.add(new UpdateDaysOnSicknessBenefitMove(parentalBenefitPlan, monthPlan, 1));
            }
        }
        return moveList;
    }
